...
"bindNavEvent":function(pathname){
	...
	$("#mw-panel").find("h5").click(function(){
		...
		if (b.is(":visible")){
			b.slideUp("fast");
			// p.removeClass("active");						if(p.find("h5").length == 1){			  p.removeClass("active");			} else {								pp = p.find("h5:first");								pp.css( "color", "#333");				pp.removeClass("active1");			}
		}else{
			b.find("li").find("em").addClass("none");
			b.slideDown("fast",function(){
				b.find("li").find("em").removeClass("none");
			});						// p.addClass("active");			if(p.find("h5").length == 1){
			  p.addClass("active");			} else {				/*				    border-top-color: #70ABD5;					border-left-color: transparent;					margin: 0 6px -3px -18px;					*margin-top: 6px				*/				var pp = p.find("h5:first");								//pp.css({"border-top-color": "#70ABD5","border-left-color": "transparent","margin": "0 6px -3px 16px","*margin-top": "6px"});				pp.css( "color", "#70ABD5");				 				var span = pp.find("span");				span.removeClass("portal_arrow");				pp.addClass("active1");				span.addClass("portal_arrow1");			}
		}
